...
def get_busline_runs(
    db: Session,
    busline_id: int,
) -> list[app.schemas.RunInDB]:
    return (
...
def del_run(
    ...
    for run_stop in run.run_stops:
        del_run_stop(db=db, run_stop=run_stop)
    db.delete(run)
    db.commit()
    return run
